...
namespace Cosplay_Academy
{
    public partial class Settings : BaseUnityPlugin
    {
        ...
        internal void StandardSettings()
        {
            ...
            {
                ListOverride[i] = Config.Bind("Outfit Folder Override", Constants.InputStrings[i].Trim('\\').Replace('\\', ' '), coordinatepath + Constants.InputStrings[i], new ConfigDescription("Choose a particular folder you wish to see used, this will be prioritzed and treated as a set\nThere is no lewd experience suport here", null, new ConfigurationManagerAttributes { IsAdvanced = true, Order = (ListOverride.Length - i) * 2 - 1 }));
                ListOverrideBool[i] = Config.Bind("Outfit Folder Override", Constants.InputStrings[i].Trim('\\').Replace('\\', ' ') + " Enable override", false, new ConfigDescription("Enables the above folder override", null, new ConfigurationManagerAttributes { IsAdvanced = true, Order = (ListOverride.Length - i) * 2 }));
            }
